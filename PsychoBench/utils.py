...
def get_questionnaire(name):
    try:
        json_path = os.path.join(os.path.dirname(__file__), 'questionnaires.json')
        with open(json_path, encoding="utf-8") as dataset:
            all_questionnaires = json.load(dataset)

        # Parcourt la liste et cherche le questionnaire par nom
        for q in all_questionnaires:
            if q.get("name") == name:
                return q

        raise KeyError(f"Questionnaire '{name}' introuvable dans questionnaires.json")

    except FileNotFoundError:
        raise FileNotFoundError("Le fichier 'questionnaires.json' est introuvable.")